...
inline std::string merge_custom_args(const std::string& high,
    ...
    const auto high_map = build_custom_args_map(parse_custom_args(high, true), canonicalize);
    const auto low_map = build_custom_args_map(parse_custom_args(low, true), canonicalize);
...
inline std::string resolve_scoped_custom_args(const ScopedCustomArgs& args,
    ...
    if (args.request_state == CustomArgsRequestState::Value) {
        return merge_custom_args(args.request, args.backend, canonicalize);
    }
    ...
    const std::string& model_args =
        args.request_state == CustomArgsRequestState::Tombstone
            ? args.model_defaults
            : args.model;
    const std::string model_scope = merge_custom_args(model_args, args.architecture, canonicalize);
    return merge_custom_args(model_scope, args.backend, canonicalize);
}
