...
private:
    ...
    // Custom llama.cpp engine registry: list registered + discovered builds,
    ...
    void handle_model_paths_post(const httplib::Request& req, httplib::Response& res);
    // Recent log lines (from the in-memory log hub) for the UI log viewer.
    void handle_logs(const httplib::Request& req, httplib::Response& res);
