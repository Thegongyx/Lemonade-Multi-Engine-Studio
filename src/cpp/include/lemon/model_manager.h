...
struct UpdateCheckResult {
    ...
    // Replace the full set of extra model roots. Multiple roots are scanned in
    // order; each contributes GGUF models to the library.
    void set_extra_models_dirs(const std::vector<std::string>& dirs);
    std::vector<std::string> extra_models_dirs() const;
...
private:
    ...
    // Scan a single extra-model root into `discovered`. Shared by the
    // multi-root discover_extra_models() loop.
    void discover_extra_models_in_root(
    ...
    std::vector<std::string> extra_models_dirs_;  // Multi-root variant of the above
