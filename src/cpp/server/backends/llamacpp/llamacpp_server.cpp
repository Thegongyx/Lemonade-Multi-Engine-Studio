...
// True when the user's custom args already name this flag or one of its aliases.
static bool custom_args_has_any(const std::vector<std::string>& tokens,
                                const std::string& key,
                                const std::vector<std::string>& aliases) {
    if (custom_args_has_flag(tokens, key)) return true;
    for (const auto& alias : aliases) {
        if (custom_args_has_flag(tokens, alias)) return true;
    }
    return false;
}

// Push a lemonade-managed flag-value pair UNLESS the user already provided it.
// A hand-written value always wins: lemonade neither injects its own value nor
// reserves the flag in that case.
static void push_managed_arg(std::vector<std::string>& args,
                             std::set<std::string>& reserved,
                             const std::vector<std::string>& user_tokens,
                             const std::string& key,
                             const std::string& value,
                             const std::vector<std::string>& aliases = {}) {
    if (custom_args_has_any(user_tokens, key, aliases)) return;
    push_arg(args, reserved, key, value, aliases);
}

// Flag-only variant of push_managed_arg.
static void push_managed_flag(std::vector<std::string>& args,
                              std::set<std::string>& reserved,
                              const std::vector<std::string>& user_tokens,
                              const std::string& key,
                              const std::vector<std::string>& aliases = {}) {
    if (custom_args_has_any(user_tokens, key, aliases)) return;
    push_arg(args, reserved, key, aliases);
}
...
void LlamaCppServer::load(const std::string& model_name,
    ...
    // the binary path, so models on the same backend type can run different
    // compiled llama.cpp builds.
    const json engine_name_json = options.get_option("llamacpp_engine");
    const std::string llamacpp_engine =
        engine_name_json.is_string() ? engine_name_json.get<std::string>() : std::string();
    const CustomEngine custom_engine = resolve_custom_engine(llamacpp_engine);
    if (!llamacpp_engine.empty() && !custom_engine.valid) {
        throw std::invalid_argument(
            "Unknown llama.cpp engine '" + llamacpp_engine +
    ...
    LOG(INFO, "LlamaCpp") << "Using LlamaCpp Backend: " << llamacpp_backend
                          << (custom_engine.valid ? (" (engine: " + llamacpp_engine + ")") : "")
                          << std::endl;
    ...
    // Install llama-server if needed (use per-model backend). A custom engine
    // ships its own binary and DLLs, so lemonade never installs it.
    ...
    std::string executable = custom_engine.valid
        ? custom_engine.path
    ...
    // Build command arguments while tracking reserved flags. The user's custom
    // args are parsed up front so managed flags can defer to hand-written ones.
    std::vector<std::string> args;
    std::set<std::string> reserved_flags;
    const std::vector<std::string> user_arg_tokens = parse_custom_args(llamacpp_args);
    ...
    push_managed_arg(args, reserved_flags, user_arg_tokens, "--ctx-size",
                     std::to_string(ctx_size), std::vector<std::string>{"-c"});

    const bool user_device =
        custom_args_has_any(user_arg_tokens, "--device", {"-dev"});
    if (!llamacpp_device.empty() && !user_device) {
    ...
    push_managed_flag(args, reserved_flags, user_arg_tokens, "--jinja",
                      std::vector<std::string>{"--no-jinja"});
    push_managed_flag(args, reserved_flags, user_arg_tokens, "--metrics");
    ...
    if (is_llamacpp_rocm_backend(llamacpp_backend)) {
        ...
        if (llamacpp_backend == "rocm-stable" && !custom_engine.valid) {
    ...
    // Merge environment variables declared by the custom engine (e.g. a plugin
    // path such as ROCMFPX_PLUGIN_PATH). Appended last so they win over the
    // generic backend defaults above.
    if (custom_engine.valid) {
