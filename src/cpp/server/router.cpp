...
RecipeOptions Router::resolve_effective_options(const ModelInfo& model_info,
    ...
    for (const auto& key : RecipeOptions::keys_for_recipe(model_info.recipe)) {
        ...
        // llama.cpp flags have many aliases (--temp/--temperature, -ngl/
        // --n-gpu-layers, …). Canonicalizing before the precedence merge makes a
        // hand-written alias collapse with the default so the higher-precedence
        // layer wins instead of both surviving.
        const utils::FlagCanonicalizer canonicalize =
            model_info.recipe == "llamacpp"
                ? utils::FlagCanonicalizer(utils::canonicalize_llamacpp_flag)
                : utils::FlagCanonicalizer();
