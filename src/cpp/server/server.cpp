...
Server::Server(std::shared_ptr<RuntimeConfig> config,
    ...
    model_manager_->set_extra_models_dirs(config_->extra_models_dirs());
...
void Server::setup_routes(httplib::Server &web_server) {
    ...
    // Engine registry: list llama.cpp builds and expose one build's --help catalog.
    register_get("engines", [this](const httplib::Request& req, httplib::Response& res) {
        handle_engines(req, res);
    });
    register_post("engines", [this](const httplib::Request& req, httplib::Response& res) {
    ...
    register_get("model-paths", [this](const httplib::Request& req, httplib::Response& res) {
        handle_model_paths_get(req, res);
    });
    register_post("model-paths", [this](const httplib::Request& req, httplib::Response& res) {
        handle_model_paths_post(req, res);
    });
    register_get("logs", [this](const httplib::Request& req, httplib::Response& res) {
        handle_logs(req, res);
    });
    for (const char* prefix : {"/api/v0", "/api/v1", "/v0", "/v1"}) {
        const std::string route = std::string(prefix) + R"(/engines/(.+)/params)";
        web_server.Get(route, [this](const httplib::Request& req, httplib::Response& res) {
            handle_engine_params(req, res);
        });
    }
...
window.api = {
    ...
    writeClipboard: async (text) => {
        ...
        // Multi-engine studio frontend is served under /app (same files as /web-app).
        web_server.Get("/app/?", serve_web_app_html);
        web_server.Get(R"(/app/(.+))", [serve_web_app_asset](const httplib::Request& req, httplib::Response& res) {
            std::string file_path = req.matches[1].str();
            serve_web_app_asset(req, res, file_path);
        });
...
// Parse `llama-server --help` into a structured parameter catalog. Option lines
// look like "-ngl,  --n-gpu-layers N   number of layers to store in VRAM"; we key
// on the long flag and treat the token after it (if any) as the value type.
nlohmann::json parse_llamacpp_help(const std::string& help) {
    nlohmann::json params = nlohmann::json::array();
    static const std::regex option_re(
        R"(--([A-Za-z0-9][A-Za-z0-9-]*)(?:[ \t]+([A-Za-z_\[][^ \t]*))?[ \t]{2,}(.*)$)");
    ...
    while (std::getline(stream, line)) {
        ...
        std::smatch match;
        if (!std::regex_search(trimmed, match, option_re)) continue;
        const std::string type_token = match[2].matched ? match[2].str() : "";
        std::string type = "bool";
        if (!type_token.empty()) {
            if (type_token.front() == '[') type = "enum";
            else if (type_token == "N") type = "int";
            else type = "string";
        }
        params.push_back({{"flag", "--" + match[1].str()},
                          {"name", match[1].str()},
                          {"type", type},
                          {"default", nullptr},
                          {"help", trim_ascii(match[3].str())}});
    }
    return params;
}
...
std::string resolve_engine_exe(const std::string& id, const nlohmann::json& section) {
    if (section.contains("custom_engines") && section["custom_engines"].is_object() &&
        ...
        return section["custom_engines"][id].value("path", std::string(""));
    }
    if (section.contains("engines_dir") && section["engines_dir"].is_string()) {
        std::error_code ec;
...
void Server::handle_engines(const httplib::Request& req, httplib::Response& res) {
    ...
    try {
        ...
        if (section.contains("engines_dir") && section["engines_dir"].is_string()) {
            ...
            std::filesystem::path root =
                lemon::utils::path_from_utf8(section["engines_dir"].get<std::string>());
        ...
        res.set_content(nlohmann::json{{"engines", engines}}.dump(), "application/json");
    } catch (const std::exception& e) {
        LOG(ERROR, "Server") << "ERROR in handle_engines: " << e.what() << std::endl;
        res.status = 500;
        res.set_content(nlohmann::json{{"error", e.what()}}.dump(), "application/json");
    }
}

void Server::handle_engine_params(const httplib::Request& req, httplib::Response& res) {
    if (req.method == "HEAD") { res.status = 200; return; }
    try {
        ...
        std::string output;
        lemon::utils::ProcessManager::run_command("\"" + exe + "\" --help", output);
        res.set_content(
            nlohmann::json{{"engine", id}, {"params", parse_llamacpp_help(output)}}.dump(),
            "application/json");
    } catch (const std::exception& e) {
        LOG(ERROR, "Server") << "ERROR in handle_engine_params: " << e.what() << std::endl;
        res.status = 500;
        res.set_content(nlohmann::json{{"error", e.what()}}.dump(), "application/json");
    }
}

void Server::handle_engine_add(const httplib::Request& req, httplib::Response& res) {
    if (req.method == "HEAD") { res.status = 200; return; }
    try {
        nlohmann::json body = nlohmann::json::parse(req.body);
        const std::string raw = body.value("path", std::string(""));
        if (raw.empty()) {
            res.status = 400;
            res.set_content(nlohmann::json{{"error", "Missing 'path'"}}.dump(), "application/json");
            return;
        }
        std::error_code ec;
        std::filesystem::path p = lemon::utils::path_from_utf8(raw);
#ifdef _WIN32
        const std::string exe_name = "llama-server.exe";
#else
        const std::string exe_name = "llama-server";
#endif
        ...
        if (std::filesystem::is_directory(p, ec)) {
            exe = p / exe_name;
            dir = p;
        } else if (p.filename().string() == exe_name && std::filesystem::exists(p, ec)) {
            exe = p;
            dir = p.parent_path();
        }
        if (exe.empty() || !std::filesystem::exists(exe, ec)) {
            res.status = 400;
            res.set_content(
                nlohmann::json{{"error", "No " + exe_name + " found in: " + raw}}.dump(),
                "application/json");
            return;
        }
        const std::string name = lemon::utils::path_to_utf8(dir.filename());
        std::string backend = "cpu";
        if (std::filesystem::exists(dir / "ggml-hip.dll", ec) ||
            std::filesystem::exists(dir / "amdhip64_7.dll", ec)) {
            backend = "rocm";
        } else if (std::filesystem::exists(dir / "ggml-vulkan.dll", ec)) {
            backend = "vulkan";
        }
        nlohmann::json entry = {{"path", lemon::utils::path_to_utf8(exe)},
                                {"backend", backend},
                                {"device", ""}};
        if (auto* cfg = RuntimeConfig::global()) {
        ...
        res.set_content(
            nlohmann::json{{"status", "success"}, {"id", name}, {"backend", backend}}.dump(),
            "application/json");
    } catch (const std::exception& e) {
        LOG(ERROR, "Server") << "ERROR in handle_engine_add: " << e.what() << std::endl;
        res.status = 500;
        res.set_content(nlohmann::json{{"error", e.what()}}.dump(), "application/json");
    }
}
...
void Server::handle_model_paths_get(const httplib::Request& req, httplib::Response& res) {
    if (req.method == "HEAD") { res.status = 200; return; }
    nlohmann::json paths = nlohmann::json::array();
    for (const auto& p : model_manager_->extra_models_dirs()) {
        paths.push_back(p);
    }
    res.set_content(nlohmann::json{{"paths", paths}}.dump(), "application/json");
}

void Server::handle_model_paths_post(const httplib::Request& req, httplib::Response& res) {
    try {
        nlohmann::json body = nlohmann::json::parse(req.body);
        if (!body.contains("paths") || !body["paths"].is_array()) {
            res.status = 400;
            res.set_content(nlohmann::json{{"error", "Body must contain a 'paths' array"}}.dump(),
                            "application/json");
            return;
        }
        ...
        // Persist so the roots survive a restart. Unknown keys are preserved by
        // ConfigFile, so this only touches extra_models_dirs.
        if (!config_dir_.empty()) {
            nlohmann::json user_cfg = ConfigFile::load_raw(config_dir_);
            nlohmann::json arr = nlohmann::json::array();
            for (const auto& p : paths) arr.push_back(p);
            user_cfg["extra_models_dirs"] = arr;
            ConfigFile::save(config_dir_, user_cfg);
        }
        model_manager_->set_extra_models_dirs(paths);

        nlohmann::json out_paths = nlohmann::json::array();
        for (const auto& p : model_manager_->extra_models_dirs()) {
            out_paths.push_back(p);
        }
        res.set_content(nlohmann::json{{"status", "success"}, {"paths", out_paths}}.dump(),
                        "application/json");
    } catch (const std::exception& e) {
        LOG(ERROR, "Server") << "ERROR in handle_model_paths_post: " << e.what() << std::endl;
        res.status = 500;
        res.set_content(nlohmann::json{{"error", e.what()}}.dump(), "application/json");
    }
}
...
void Server::handle_logs(const httplib::Request& req, httplib::Response& res) {
    ...
    if (req.has_param("tail")) {
        try {
            tail = static_cast<size_t>(std::stoul(req.get_param_value("tail")));
        } catch (...) {
            tail = 200;
        }
    }
    if (tail == 0 || tail > 5000) tail = 200;
    const std::string filter = req.has_param("q") ? req.get_param_value("q") : "";

    std::vector<LogStreamEntry> snapshot;
    const std::string sub = LogStreamHub::instance().subscribe_with_snapshot(
        [](const LogStreamEntry&) {}, std::nullopt, snapshot);
    LogStreamHub::instance().remove_subscriber(sub);

    nlohmann::json lines = nlohmann::json::array();
    const size_t start = snapshot.size() > tail ? snapshot.size() - tail : 0;
    for (size_t i = start; i < snapshot.size(); ++i) {
        const auto& entry = snapshot[i];
        if (!filter.empty() && entry.line.find(filter) == std::string::npos &&
            entry.tag.find(filter) == std::string::npos) {
            continue;
        }
        lines.push_back({{"timestamp", entry.timestamp},
