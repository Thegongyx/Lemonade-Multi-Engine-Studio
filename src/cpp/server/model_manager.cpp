...
// HF hub cache layout: <root>/models--<org>--<repo>/snapshots/<commit>/file.gguf.
// The leaf directory is a commit hash, which is meaningless as a model name, so
// derive the name from the repo directory when the path is inside an HF cache.
static std::string hf_repo_name_from_path(const fs::path& dir_path) {
    const std::string prefix = "models--";
    for (fs::path p = dir_path; !p.empty(); p = p.parent_path()) {
        const std::string name = p.filename().string();
        if (name.rfind(prefix, 0) == 0) {
            const std::string rest = name.substr(prefix.size());
            const auto sep = rest.find("--");
            return sep == std::string::npos ? rest : rest.substr(sep + 2);
        }
...
void ModelManager::set_extra_models_dirs(const std::vector<std::string>& dirs) {
    extra_models_dirs_ = dirs;

    directory_watcher_.reset();
    if (!extra_models_dirs_.empty()) {
        for (const auto& d : extra_models_dirs_) {
            LOG(INFO, "ModelManager") << "Extra models directory set to: " << d << std::endl;
        }
        start_directory_watcher();
    }
...
void ModelManager::start_directory_watcher() {
    std::string watch_dir = extra_models_dir_;
    if (watch_dir.empty() && !extra_models_dirs_.empty()) {
        watch_dir = extra_models_dirs_.front();
    }
    directory_watcher_ = std::make_unique<DirectoryWatcher>(watch_dir);
...
std::map<std::string, ModelInfo> ModelManager::discover_extra_models() const {
    ...
    for (const auto& root : roots) {
        ...
        discover_extra_models_in_root(search_path, discovered);
    }

    if (!roots.empty()) {
        LOG(INFO, "ModelManager") << "Discovered " << discovered.size()
                                  << " models from extra directories" << std::endl;
    }
    return discovered;
}
...
void ModelManager::discover_extra_models_in_root(
    ...
    LOG(INFO, "ModelManager") << "Scanning for GGUF models in: " << path_to_utf8(search_path) << std::endl;
    ...
    try {
        ...
        LOG(ERROR, "ModelManager") << "Error scanning directory " << path_to_utf8(search_path) << ": " << e.what() << std::endl;
        return;
    }
...
void ModelManager::discover_extra_models_in_directory(
    ...
    // HF cache leaf dirs are commit hashes; prefer the repo name instead.
    const std::string hf_name = hf_repo_name_from_path(dir_path);
    if (!hf_name.empty()) dir_name = hf_name;
