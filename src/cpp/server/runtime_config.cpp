...
std::vector<std::string> RuntimeConfig::extra_models_dirs() const {
    std::shared_lock lock(mutex_);
    std::vector<std::string> dirs;
    if (config_.contains("extra_models_dirs") && config_["extra_models_dirs"].is_array()) {
        for (const auto& entry : config_["extra_models_dirs"]) {
            if (entry.is_string()) dirs.push_back(entry.get<std::string>());
        }
    }
    return dirs;
}
...
void RuntimeConfig::validate_backend(const std::string& backend, const std::string& key,
    ...
    else if (key == "custom_engines") {
        if (!value.is_object()) {
            throw std::invalid_argument("'" + backend + "." + key + "' must be an object");
        }
    }
    else if (key == "engines_dir") {
        if (!value.is_string()) {
            throw std::invalid_argument("'" + backend + "." + key + "' must be a string");
        }
    }
